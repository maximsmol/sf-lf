...
From LF Require Export Tactics_done.
...
Proof.
  ...
  - destruct m. reflexivity. rewrite plus_comm in H. inversion H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros HR. apply HQR in HR. apply HPQ in HR. apply HR.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [EHx EH].
    destruct EH.
    + left. exists EHx. apply H.
    + right. exists EHx. apply H.
  - intros [[HPx HP] | [HQx HQ]].
    + exists HPx. left. apply HP.
    + exists HQx. right. apply HQ.
Qed.
(** [] *)
...
Proof.
  intros A B f l.
  induction l.
  - simpl. split. intros []. intros [Hx [Hxy []]].
  - simpl. split.
    + intros [H | H].
      * exists x. split. apply H. left. reflexivity.
      * apply IHl in H. destruct H as [Hx H]. exists Hx. destruct H as [Hf Hin]. split. apply Hf. right. apply Hin.
    + intros [Hx [Ha Hb]].
      destruct Hb as [HxHx | Hin].
      * left. rewrite <- HxHx in Ha. apply Ha.
      * rewrite IHl. right. exists Hx. split. apply Ha. apply Hin.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H.
    ...
    + simpl in H. simpl.
      destruct H as [Hxa | Hin].
      * left. left. apply Hxa.
      * apply IHl in Hin. destruct Hin as [H0 | H1]. left. right. apply H0. right. apply H1.
  - intros H.
    induction l.
    + simpl. destruct H as [[] | H]. apply H.
    + simpl. simpl in H. rewrite <- or_assoc in H.
      destruct H.
      * left. apply H.
...
Proof.
  ...
  - intros H.
    induction l.
    + simpl. apply I.
    + simpl. split. apply H. simpl. left. reflexivity. apply IHl. intros x0 HH. apply H. simpl. right. apply HH.
  - intros HAll x Hin.
    induction l.
    + simpl in Hin. destruct Hin.
    + simpl in Hin. simpl in HAll.
      destruct Hin.
      * destruct HAll as [HAll _]. rewrite H in HAll. apply HAll.
      * apply IHl. destruct HAll as [_ HAll]. apply HAll. apply H.
Qed.
(** [] *)
...
Lemma rev_append_correct : forall (X : Type) (a b : list X), rev_append a b = rev a ++ b.
Proof.
  intros X a.
  induction a as [|x a].
  - reflexivity.
  - intros b. simpl.
    rewrite <- (app_assoc X (rev a) [x] b).
    simpl. rewrite (IHa (x :: b)). reflexivity.
Qed.
...
Proof.
  ...
  destruct xs as [| x xs].
  - reflexivity.
  - simpl. rewrite (rev_append_correct X xs [x]). reflexivity.
Qed. 
...
Proof.
  ...
  - induction n.
    + exists 0. reflexivity.
    + destruct (evenb (S n)) eqn: Heven.
      * rewrite evenb_S. rewrite Heven. simpl. destruct IHn as [k IHk].
        exists k. rewrite IHk. reflexivity.
      * rewrite evenb_S. rewrite Heven. simpl. destruct IHn as [k IHk].
        rewrite IHk. exists (S k). reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  - intros H.
    split.
    + rewrite andb_commutative in H. apply andb_true_elim2 in H. rewrite H. reflexivity.
    + apply andb_true_elim2 in H. rewrite H. reflexivity.
  - intros [H1 H2].
...
Proof.
  ...
  - intros H.
    destruct b1.
    + left. reflexivity.
    + right. destruct b2. reflexivity. inversion H.
  - intros [H1 | H2].
...
Proof.
  ...
  - unfold not.
    intros Heq H.
    rewrite H in Heq.
    rewrite <- beq_nat_refl in Heq. inversion Heq.
  - intros H.
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  match l1, l2 with
    | [], [] => true
    | a :: l1', b :: l2' => beq a b && beq_list beq l1' l2'
    | _, [] => false
    | [], _ => false
  end.
  ...
  - generalize dependent l2.
    ...
    + destruct l2.
      ...
      * simpl. rewrite andb_true_iff.
        intros H. destruct H as [Hel Hl].
        rewrite Heq in Hel.
        apply IHl1 in Hl.
        rewrite Hel. rewrite Hl.
        reflexivity.
  - generalize dependent l2.
    induction l1.
    + destruct l2.
    ...
    + destruct l2.
      ...
      * intros H.
        ...
        rewrite H2 in IHl1. apply IHl1. reflexivity.
Qed.
...
Theorem forallb_false_iff : forall X test (l : list X),
  forallb test l = false <-> exists x, test x = false /\ In x l.
Proof.
  intros X test l.
  split.
  - induction l.
    ...
    + simpl.
      ...
      * intros H. simpl in H.
        apply IHl in H.
        destruct H as [x0 [IHxtest IHxIn]].
        exists x0.
        split. apply IHxtest. right. apply IHxIn.
      * intros H.
        exists x. split. apply Htest. left. reflexivity.
  - induction l.
    + intros [x [Htest Hin]]. inversion Hin.
    + simpl.
      intros [x0 [Htest H]].
      destruct H as [Ha | Hb].
      rewrite Ha. rewrite Htest. reflexivity.
      rewrite IHl.
      * apply andb_false_r.
      * exists x0. split. apply Htest. apply Hb.
Qed.
...
Proof.
  ...
  intros H.
  intros x.
  unfold excluded_middle in M. unfold not in M.
  destruct (M (P x)) as [Ha | Hb].
  - apply Ha.
  - exfalso. apply H. exists x. apply Hb.
Qed.
(** [] *)
...
Theorem excluded_middle_iff_double_neg_elim :
  excluded_middle <-> double_negation_elimination.
Proof.
  split.
  - intros M. unfold excluded_middle in M. unfold not in M.
  ...
  - intros DN. unfold double_negation_elimination in DN.
...
Proof.
  split.
  - intros M. unfold excluded_middle in M.
    unfold peirce.
    intros P Q H.
    destruct (M P).
    + apply H0.
...
Theorem excluded_middle_iff_de_morgan_not_and_not :
...
Theorem excluded_middle_iff_implies_to_or :
...
Proof.
  ...
  - intros M. unfold excluded_middle in M.
    ...
    + left. apply H0.
  - intros IO. unfold implies_to_or in IO.
    unfold excluded_middle.
    intros P.
    apply or_commut.
    apply IO.
