...
Lemma ev'_means_ev : forall n, ev' n -> ev n.
Proof.
  intros n H'.
  induction H' as [| | n m H'n Hn H'm Hm].
  - apply ev_0.
...
Proof.
  ...
  - apply ev'_means_ev.
  - intros H.
    induction H as [| n Hn H'n].
    + apply ev'_0.
    + induction H'n as [| | n m H'n IHn H'm IHm].
      * apply ev'_2.
      * apply ev'_sum with (n:=2) (m:=2). apply ev'_2. apply ev'_2.
      * rewrite <- plus_Sn_m. rewrite <- plus_Sn_m. apply ev'_sum.
        { apply IHn. apply ev'_means_ev in H'n. apply H'n. }
        { apply H'm. }
Qed.
(** [] *)
...
Proof.
  ...
  replace (n + m + (n + p)) with ((m + p) + (n + n)) in Hnm.
  rewrite <- double_plus in Hnm.
  apply ev_ev__ev with (m:=m+p) in Hnm.
  apply Hnm.
  replace (m + p + double n + (m + p)) with (double (m + p + n)).
  apply ev_double.
  rewrite double_plus.
  rewrite double_plus.
  rewrite plus_assoc.
  ...
  rewrite plus_assoc.
  rewrite plus_assoc.
  rewrite <- (plus_assoc (m + p + n) n m).
  rewrite (plus_comm n m).
  rewrite plus_assoc.
  rewrite <- (plus_assoc _ n p).
...
Inductive total_relation : nat -> nat -> Prop :=
  | total : forall a b, total_relation a b.
(** [] *)
...
Inductive empty_relation : nat -> nat -> Prop :=
  | empty : forall a b, False -> empty_relation a b.
(** [] *)
