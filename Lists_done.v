...
From LF Require Export Induction_done.
Module NatList.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | [] => true
  | a :: t =>
    match member a s2 with
    | true => subset t (remove_one a s2)
    | false => false
    end
  end.
...
Theorem eq_reflexive : forall n : nat,
  beq_nat n n = true.
Proof.
  intros n.
...
Theorem add_plus_count : forall (n : nat) (s : bag),
...
Proof.
  induction l as [| n l' IHl'].
  - reflexivity.
  - simpl. rewrite -> eq_reflexive. rewrite -> IHl'. reflexivity.
Qed.
(** [] *)
...
Theorem sum_plus_count : forall (s1 s2 : bag),
