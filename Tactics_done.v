...
From LF Require Export Poly_done.
...
induction
n can be O (base):
  Prove: forall m, beq_nat O m = true -> O = m.

  Take some m.
  m can be O:
    Prove: beq_nat O O = true -> O = O. trivial.
  m can be S m':
    Prove: beq_nat O (S m') = true -> O = S m'. O != S _ => explosion.

n can be S n' (step):
  Induction H: forall m, beq_nat n' m = true -> n' = m.
  Prove: forall m, beq_nat (S n') m = true -> S n' = m.

  Take some m.
  m can be O:
    Prove:  beq_nat (S n') O = true -> S n' = m.
    beq_nat (S n') O = false (= true). => explosion.
  m can be S m':
    Prove: beq_nat (S n') (S m') = true -> S n' = S m'.
    S a = S b -> a = b
    Prove: beq_nat (S n') (S m') = true -> n' = m'.
    apply H with m = m'.
    Prove: beq_nat (S n') (S m') = true -> beq_nat n' m' = true.
    beq_nat (S n') (S m') = beq_nat n' m'.
    Use beq_nat definition: beq_nat (S a) (S b) = beq_nat a b
...
Proof.
  ...
  - intros l1 l2. simpl. destruct x as [a b]. destruct (split l) as (lx, ly).
    intros H.
    inversion H.
    simpl.
    apply f_equal.
...
Proof.
  ...
  induction l as [|a l].
  - simpl. intros x lf H. inversion H.
  - intros x lf.
    simpl.
    destruct (test a) eqn:Htest.
    + intros H. inversion H.
      rewrite <- H1. apply Htest.
    + apply IHl.
Qed.
...
Fixpoint forallb {X : Type} (f : X -> bool) (l : list X) :=
  match l with
  | [] => true
  | a :: t => if f a then forallb f t else false
  end.

Fixpoint existsb {X : Type} (f : X -> bool) (l : list X) :=
  match l with
  | [] => false
  | a :: t => if f a then true else existsb f t
  end.
